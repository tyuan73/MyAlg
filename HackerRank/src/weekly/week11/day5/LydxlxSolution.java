...
public class LydxlxSolution {
    ...
    static class SplayTree {
        ...
        private void dfs(Node root, int total, int[] buffer) {
            if (root == nil) return;
            if (buffer[0] >= total) return;

            pushDown(root);
            dfs(root.child[0], total, buffer);
            if (buffer[0] < total) buffer[++buffer[0]] = root.value;
            dfs(root.child[1], total, buffer);
        }

        public void collect(int left, int right, int[] buffer) {
            reverse(right);
            buffer[0] = 0;
            dfs(root, right - left + 1, buffer);
            reverse(right);
        }
