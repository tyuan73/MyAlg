...
/**
 * Created by yuantian on 11/7/14.
 */
...
public class BurgerHappinessUWISolution {
    ...
    static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            long val = 0;
            val = Math.max(val, -sstlr.min(0, shop[i][0]) + sstlr.min(shop[i][0], shop[i][0] + 1));
            val = Math.max(val, -sstrl.min(shop[i][0] + 1, n + 1) + sstrl.min(shop[i][0], shop[i][0] + 1));
            val += shop[i][1];
    ...
    public static int[] shrinkXI(int[][] co, int[] dim) {
        int n = co.length;
        int m = dim.length;
        int p = 0;
        long[] b = new long[n * m];
