...
public class MonstersValley
{
    public int minimumPrice(long[] dread, int[] price)
    {
        ...
        for(int i = 0; i < n; i++) {
            r += price[i];
            for(int j = r; j >= 0; j--) {// j is price to pay
                long temp = j >= price[i] && dp[j-price[i]] >= 0? dp[j-price[i]]+dread[i]: -1;
                if(dp[j] >= dread[i])      // very important, it's ">=" not ">"
	...
	*/
	...
	public static void main(String[] args)
	{
		...
		System.out.println();
        time = System.currentTimeMillis();
        answer = new MonstersValley().minimumPrice(new long[]{1L, 1L}, new int[]{1, 1});
        System.out.println("Time: " + (System.currentTimeMillis()-time)/1000.0 + " seconds");
        desiredAnswer = 1;
        System.out.println("Your answer:");
        System.out.println("\t" + answer);
