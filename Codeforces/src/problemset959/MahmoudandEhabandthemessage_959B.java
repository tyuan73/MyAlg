...
public class MahmoudandEhabandthemessage_959B {
    static void go() {
        ...
        for (int i = 0; i < k; i++) {
            ...
            int a = find(ds, in.nextInt());
            while (num-- > 0) {
                //int y = in.nextInt();
                int b = find(ds, in.nextInt());
                if (a != b) {
        ...
        for (int i = 0; i < m; i++) {
            ...
            int idx = map.get(in.nextString());
            ans += cost[find(ds, idx) - 1];
        }
        out.println(ans);
    }
