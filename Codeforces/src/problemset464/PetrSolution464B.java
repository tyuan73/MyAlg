...
public class PetrSolution464B {
    ...
    static private boolean matches(int[][] need, int[][] got, int[] which, int at) {
        if (at == 4) return true;
    ...
    static private void printPoint(PrintWriter out, int[] coord, int[] perm) {
    ...
    static private void fillCell(int[] dest, int[] a, int[] b, int[] c, int[] d) {
        dest[0] = a[0] + b[0] + c[0] + d[0];
        dest[1] = a[1] + b[1] + c[1] + d[1];
        dest[2] = a[2] + b[2] + c[2] + d[2];
    }

    static private long dotprod(int[] d1, int[] d2) {
